...
It is possible to make maps of sub-national divisions by defining them via `region`. For example, `states("sw")` returns a character vector of the states in the South-West geopolitical zone (see `?states`). To display them on a map with a few embellishments like labelling and colour, write
...
The function `map_ng` carried out a bounds check on point data and returns an error, and the points are not plotted, when the coordinates care beyond the region. For instance, using the same points provided earlier, if we try to plot them on a map of Kwara State,
...
## Conclusion
This vignette provided an initial look at the mapping facilities currently available in the `naijR` package. Additional development is going on to provide the user with more granular control. This is an open source project, and contributions are welcome.